<template>
  <div class="min-h-screen bg-gradient-to-br from-gray-100 to-white">
    <!-- Tombol Keranjang -->
    <button @click="router.push('/transaksi')"
      ...
      🧺 Keranjang
      <span v-if="keranjangCount > 0" class="bg-white text-blue-600 text-xs font-bold rounded-full px-2 py-0.5 shadow">
        {{ keranjangCount }}
      </span>
    </button>
    ...
    <div class="pt-10 pb-16 px-6 max-w-6xl mx-auto space-y-14">
      ...
      <!-- List Kategori Menu -->
      <div v-for="menu in menuStore.daftarMenu" :key="menu.id"
        class="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 space-y-4">
        <!-- Judul Kategori -->
        <div class="flex items-center gap-3 mb-4">
          <div
            class="w-10 h-10 bg-gradient-to-br from-blue-500 to-teal-500 rounded-xl flex items-center justify-center text-white">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          <div v-for="item in menu.variasi" :key="item.id"
            ...
            <button @click="tambahKeKeranjang(item)"
